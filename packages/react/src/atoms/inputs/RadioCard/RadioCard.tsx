  ...
  if (skeleton) {
    return (
      <span className={cx(styles.skeleton, className)}>
        <Skeleton variant={SkeletonVariant.Text} width="7rem" />
        <Skeleton variant={SkeletonVariant.Text} width="10rem" />
      </span>
    );
  }
  ...
  return (
    <label className={cx(styles.card, controlled && checked && styles.checked, disabled && styles.disabled, className)}>
      <input
        ...
        // A native radio group manages checked state across its siblings, so an
        // uncontrolled card must be a native (defaultChecked) input. Controlling
        // `checked` here would make React fight the browser over the group and the
        // non-clicked cards would keep stale state - the source of the laggy,
        // hard-to-swap selection. Controlled cards pass `checked`; the parent then
      ...
      <RadioCardIndicator checked={controlled ? checked : undefined} reduce={reduce} />
    </label>
  );
}
