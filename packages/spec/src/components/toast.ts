...
export const toastSpec: ComponentSpec = {
  ...
  tones: [
    { name: 'neutral', description: 'The default, low-emphasis raised surface; announces as a status.', paint: { background: token('surface-raised'), border: token('border-subtle'), text: token('text') } },
    { name: 'info', description: 'Neutral-informational tint; announces as a status.', paint: { background: token('info-soft'), border: token('info-border'), text: token('info-text') } },
    { name: 'success', description: 'Positive or complete states; announces as a status and auto-dismisses fastest.', paint: { background: token('success-soft'), border: token('success-border'), text: token('success-text') } },
    { name: 'warning', description: 'Caution states; announces as a status.', paint: { background: token('warning-soft'), border: token('warning-border'), text: token('warning-text') } },
    { name: 'danger', description: 'Errors and destructive states; announces as an alert and lingers longest before auto-dismiss.', paint: { background: token('danger-soft'), border: token('danger-border'), text: token('danger-text') } },
  ],
  ...
  states: [
    { name: 'enter', description: 'The pill slides up and fades in from y 12 on a snappy spring when it mounts; motion only, the tone paint never changes.', behavioral: true },
    { name: 'exit', description: 'The pill fades and drops back to y 12 as it is replaced, dismissed, or times out; motion only, the tone paint never changes.', behavioral: true },
    { name: 'replaced', description: 'A new toast takes the single slot immediately; there is no queue, latest wins, and the auto-dismiss timer re-arms for the new pill.', behavioral: true },
    { name: 'auto-dismiss', description: 'After its duration elapses the provider clears the toast unless a newer one already replaced it; a duration of 0 disables the timer.', behavioral: true },
    ...
    { name: 'dismiss-focus', description: 'The dismiss button shows a hairline-wide currentColor outline offset 2px on keyboard focus, and its opacity goes to 1.', tokens: { 'outline-width': token('hairline') } },
  ],
  tokens: [
