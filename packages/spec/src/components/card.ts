...
export const cardSpec: ComponentSpec = {
  ...
  variants: [
    { name: 'solid', description: 'Opaque raised surface with a subtle hairline border.', tokens: { background: token('surface-raised'), border: token('border-subtle') } },
    { name: 'glass', description: 'Translucent blurred material for chrome over content.', tokens: { background: token('glass-regular'), border: token('glass-border'), highlight: token('glass-highlight') } },
  ],
  ...
  states: [
    ...
    { name: 'hover', description: 'When interactive, the shadow bumps one step and the card lifts 2px.' },
  ],
  tokens: [
