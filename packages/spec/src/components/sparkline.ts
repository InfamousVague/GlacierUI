...
export const sparklineSpec: ComponentSpec = {
  ...
  props: [
    ...
    { name: 'glass', type: 'boolean', default: false, description: 'Mounts the mark on the frosted glass material: a rounded, blurred tile. The tile adds its padding around the sized mark box.' },
  ...
  defaults: { shape: 'line', tone: 'accent', size: 'md', endPoint: false, glass: false, skeleton: false },
  dimensions: {
    barGap: token('hairline'),
    baselineWidth: token('hairline'),
    pointDiameter: '0.375rem',
    glassRadius: token('radius-md'),
    glassPaddingInline: token('space-2'),
    glassPaddingBlock: token('space-1'),
  },
  states: [
    ...
    {
      name: 'glass',
      description: 'The mark sits on the frosted material: blurred, saturated, with an inset top highlight.',
  ...
  tokens: [
    ...
    'hairline', 'radius-md', 'space-1', 'space-2',
