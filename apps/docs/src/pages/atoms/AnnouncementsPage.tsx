...
/>`}
      ...
      <Example
        title={t(m.anEx4Title)}
        description={prose(t(m.anEx4Desc))}
        component="Announcements"
        render={(K) => <K.Announcements items={updates} motion="marquee" secondsPerItem={6} />}
        code={`<Announcements items={updates} motion="marquee" secondsPerItem={6} />`}
      />

      <Example
        title={t(m.anEx5Title)}
        description={prose(t(m.anEx5Desc))}
        component="Announcements"
        render={(K) => (
          <K.Announcements items={updates} motion="marquee" onItemSelect={(item) => window.alert(String(item.id))} />
        )}
        code={`const [open, setOpen] = useState<AnnouncementItem | null>(null);

<Announcements items={updates} motion="marquee" onItemSelect={setOpen} />
  ...
  {open?.content}
      ...
      <PropsTable
        props={[
          ...
          { name: 'motion', type: "'step' | 'marquee'", default: "'step'", description: t(m.anPropMotion) },
          ...
          { name: 'onItemSelect', type: '(item: AnnouncementItem, index: number) => void', description: t(m.anPropOnItemSelect) },
          ...
          { name: 'secondsPerItem', type: 'number', default: '7', description: t(m.anPropSecondsPerItem) },
