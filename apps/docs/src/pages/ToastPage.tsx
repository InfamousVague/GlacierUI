import { Button, Row, Stack, Toast, ToastProvider, useToast } from '@perfect/react';
...
function ToneDemo() {
  ...
  return (
    <Row gap={3} wrap>
      <Button variant="soft" onClick={() => toast({ message: 'Draft saved to your library.' })}>
        Neutral
      </Button>
      <Button variant="soft" onClick={() => toast({ tone: 'info', message: 'A new version is available.' })}>
        Info
      </Button>
      <Button variant="soft" onClick={() => toast({ tone: 'success', message: 'Your changes were published.' })}>
        Success
      </Button>
      <Button variant="soft" onClick={() => toast({ tone: 'warning', message: 'You are close to your storage limit.' })}>
        Warning
      </Button>
      <Button variant="soft" onClick={() => toast({ tone: 'danger', message: 'Could not reach the server.' })}>
        Danger
      </Button>
    </Row>
  );
...
function IconDemo() {
  ...
  return (
    <Button
      variant="soft"
      onClick={() =>
        toast({ tone: 'success', message: 'Copied to clipboard.', icon: <span aria-hidden>✓</span> })
      }
    >
    ...
    <>
      ...
      <ToastProvider>
        <Example
          ...
          code={`import { ToastProvider, useToast } from '@perfect/react';
...
dismiss();`}
      ...
      <Example
        ...
        code={`<Toast tone="success" message="Your changes were published." />
<Toast tone="danger" message="Could not reach the server." />
<Toast skeleton message="" />`}
      >
        <Stack gap={3} style={{ width: '100%', maxWidth: '28rem' }}>
          <Toast tone="success" message="Your changes were published." dismissible={false} />
          <Toast tone="danger" message="Could not reach the server." dismissible={false} />
          <Toast skeleton message="" />
        </Stack>
      </Example>
